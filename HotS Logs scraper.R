...
test %>%
  group_by(Specialty) %>%
  summarize(mean(Win.Percent)) %>%

# Clean up scraped test
test$hero <- gsub('%20', ' ', test$hero)
test$hero <- gsub('%27', "'", test$hero)
test$total.games <- gsub(',', '', test$total.games)
test$win.percent <- gsub('%', '', test$win.percent)
test$Map.Name <- as.factor(test$Map.Name)
test$total.games <- as.numeric(test$total.games)
...
# F around
test <- table4[,2:6]
colnames(test) <- c('hero', 'total.games', 'win.percent', 'role', 'specialty')

# This is the correct way to calculate the group mean by de-weighing few games played with certain heroes
test %>%
  group_by(specialty) %>%
  summarize(count = sum(total.games*win.percent)/sum(total.games))

# This gives a different result from the above formula where win % isn't weighted by number of matches played
test%>%
  filter(specialty == 'Bruiser') %>%
  summarize(mean(win.percent))


ggplot(bh, aes(x = Win.Percent, y = Games.Played, label = hero, color = Role)) +
  geom_vline(xintercept = .5, alpha = 0.75, lty = 2, size = 1) +
  geom_text_repel() +
  theme_fivethirtyeight() +
  labs(title = paste0('Hero league win rate on ', bh$Map.Name),
       subtitle = paste0('Hero league win rate across all leagues for the last 7 days. Last updated: ', Sys.time(), ' CST.'),
       caption = '@MattDaviz                                                                                                                                                                                         Source: HOTS LOGS') +
  scale_x_continuous(limits = c(.2,.8),labels = scales::percent) +
...
whj$hero <- sapply( strwrap(whj$hero, 3, simplify=FALSE), paste, collapse="\n" )

ggplot(data = whj, aes(x = hero, y = Win.Percent, fill = Role, group = Role)) +
  geom_bar(stat = 'identity', width = 0.5) +
  coord_flip() +
  theme(legend.position = 'none') +
  scale_y_continuous(limits = c(0,.65), breaks = c(seq(0,.6,.1)),labels = scales::percent) +
  geom_hline(yintercept = .5, alpha = .75, lty = 2) +
  facet_grid(Role ~ ., scales = "free_y", space = "free_y") +
  theme_fivethirtyeight() +
  labs(title = 'Hero league win rate on ',
       subtitle = 'Hero league win rate across all leagues for the last 7 days. Last updated: ',
       caption = '@MattDaviz                                                                                                                   Source: HOTS LOGS') +
  theme(legend.position = 'none',
...
ggplot(mapdata, aes(x = hero, y = Win.Percent, group = Map.Name, color = Map.Name)) +
  geom_point() +
  coord_flip() +
  theme(legend.position = 'none') +
  facet_grid(Role.y ~ ., scales = "free", space = "free") +
  theme_fivethirtyeight() +
  theme(legend.position = 'none') +
...
ggplot(data = boe, aes(x = hero, y = Win.Percent, fill = Role, group = Role, alpha = Games.Played)) +
  geom_bar(stat = 'identity', width = 0.5) +
  coord_flip() +
  theme(legend.position = 'none') +
  scale_y_continuous(limits = c(0,.65), breaks = c(seq(0,.6,.1)),labels = scales::percent, sec.axis = dup_axis()) +
  geom_hline(yintercept = .5, alpha = .75, lty = 2) +
  facet_grid(Role ~ ., scales = "free_y", space = "free_y") +
  theme_fivethirtyeight() +
  #scale_fill_continuous() +
  labs(title = paste0('Hero league win rate on '),
       subtitle = paste0('Hero league win rate across all leagues for the last 7 days.\nLast update: ', Sys.time(), ' CST.'),
       caption = '@MattDaviz                                                                                                                   Source: HOTS LOGS') +
  theme(legend.position = 'none',
        axis.title = element_text(face = 'bold')) +
  xlab('') +
